...
class HomeController extends GetxController {
  ...
  void changeTab(int index) {
    ...
    if (wasOnReels && !goingToReels) {
      // Leaving reels — pause but keep position.
      reelCtrl.pauseCurrentVideo();
    } else if (!wasOnReels && goingToReels) {
      // Returning to reels — resume from where they left off.
      reelCtrl.resumeCurrentVideo();
    }
  }

  /// Called when user double-taps the Reels tab — refresh with new shuffled videos.
