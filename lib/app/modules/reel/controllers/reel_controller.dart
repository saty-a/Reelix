...
/// Manages video playback lifecycle only (SRP).
/// Depends on ReelRepository abstraction, not Firestore (DIP).
class ReelController extends GetxController with WidgetsBindingObserver {
  ...
  /// Bumped whenever a controller is added/removed so Obx widgets rebuild.
  ...
  void _listenConnectivity() {
    _connectivitySub =
      ...
      if (isOffline.value && !offline) {
        // Back online — reinitialize current video if broken
  ...
  Future<void> _recoverCurrentVideo() async {
    ...
    // If the current controller has errored or isn't initialized, rebuild it
  ...
  Future<void> _fetchReels() async {
    try {
      ...
      if (reels.isNotEmpty) {
        // Load any videos already on disk into the cache map
  ...
  /// Pause current video when user switches away from Reels tab.
  void pauseCurrentVideo() {
    _pauseVideo(currentIndex.value);
    isVideoPlaying.value = false;
  }

  /// Resume current video when user returns to Reels tab.
  void resumeCurrentVideo() {
    _ensureCurrentVideo(currentIndex.value);
    isVideoPlaying.value = true;
  }

  /// Double-tap refresh — dispose everything, fetch new shuffled set.
  ...
  /// Pre-cache the next [_preCacheCount] videos to disk for offline use.
